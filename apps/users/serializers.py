...
class InviteRegistrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['token', 'first_name', 'last_name', 'password', 'phone']
    ...
    def validate(self, attrs):
        ...
        if invitation.company.members.filter(is_active=True).count() >= invitation.company.max_employees:
            raise serializers.ValidationError({'detail': 'Employee limit reached'})
        attrs['invitation'] = invitation
        return attrs

    def create(self, validated_data):
        token = validated_data.pop('token')
        validated_data.pop('invitation', None)
        ...
        with transaction.atomic():
            ...
            validated_data['company'] = invitation.company
